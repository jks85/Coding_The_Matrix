...
def getitem(M, k):
    ...
    return M.f[(k[0],k[1])] if (k[0],k[1]) in M.f.keys() else 0 # M.f.keys() uses sparsity; doesn't check full domain
...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    return all([A[r,c] == B[r,c] for r in A.D[0] for c in A.D[1]]) # compare corresponding entries
...
def add(A, B):
    ...
    """
    assert A.D == B.D
    return Mat((A.D[0],A.D[1]), {(r,c):(A[r,c] + B[r,c]) for r in A.D[0] for c in A.D[1]})
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat((M.D[0],M.D[1]),{(r,c):(x*M[r,c]) for (r,c) in M.f.keys()}) #uses sparsity to scale (non-zero) vals
...
def vector_matrix_mul(v, M):
    ...
    """
    assert M.D[0] == v.D
    return Vec(M.D[1], {i:sum([v[r]*M[r,c] for (r,c) in M.f.keys() if c == i]) for i in M.D[1]})
    # set mat col domain as new vec domain. iterate over this domain to set vec dictionary function
    # list comprehension iterates over non-zero keys of M function domain and multiply desired entries of vec and mat
def matrix_vector_mul(M, v):
    ...
    """
    assert M.D[1] == v.D
    return Vec(M.D[0],{i:sum([M[r,c]*v[c] for (r,c) in M.f.keys() if r == i]) for i in M.D[0]})
...
def matrix_matrix_mul(A, B):
    ...
    """
    assert A.D[1] == B.D[0]
    return Mat((A.D[0],B.D[1]),{(i,j):sum([A[r,c]*B[c,k] for (r,c) in A.f.keys() for k in B.D[1] if r == i and k == j])
