...
S = {-4, -2, 1, 2, 5, 0}
# Replace [ ... ] with a one-line list comprehension in which S appears
exclude_zero_list = [(i,j,k) for i in S for j in S for k in S if (sum((i,j,k))==0) & ((i,j,k) != (0,0,0))]
...
first_of_tuples_list = [(i,j,k) for i in S for j in S for k in S if (sum((i,j,k))==0) & ((i,j,k) != (0,0,0))][0]
...
value_list_modified_1 = [(x[k] if k in x.keys() else 'NOT PRESENT') for x in dlist] # <-- Use the same expression here
k = 'Frodo'
value_list_modified_2 = [(x[k] if k in x.keys() else 'NOT PRESENT') for x in dlist] # <-- as you do here
...
representation_dict = {m:n for (m,n) in zip(range(base**3), [(i,j,k) for i in digits for j in digits for k in digits])}
...
# not one line-- will fix later
def all_3_digit_numbers(base, digits):
    all_tuples = [(i, j, k) for i in range(len(digits)) for j in range(len(digits)) for k in range(len(digits))]
    nums = [i * (base ** 2) + j * (base ** 1) + k * (base ** 0) for (i, j, k) in all_tuples]
    return set(nums)
