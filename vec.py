...
def dot(u,v):
    ...
    return sum([getitem(v,k)*getitem(u,k) for k in u.D if (k in u.f.keys() and k in v.f.keys())])
    # takes product of corresponding keys if both keys are in the function dictionary
    # if a key is missing then the value is 0 so it will not contribute to the dot product
...
def scalar_mul(v, alpha):
    ...
    True
    """
    return Vec(v.D,{key:(alpha*val) for (key,val) in v.f.items()})
    # return Vec(v.D,{d:(alpha*getitem(v,d)) for d in v.D})  this does not use sparsity.
    # 0*alpha = 0 so it doesn't need to be multiplied. can use v.items(
...
def neg(v):
    ...
    True
    """
    return scalar_mul(v, -1) # call scalar multiple with alpha = -1
    # return Vec(v.D,{(key:(-1*val) for (key,val) in v.f.items())}) directly creates -v and maintains sparsity
